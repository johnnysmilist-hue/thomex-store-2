...
import { ChevronLeft, ChevronRight, Smartphone, ChevronRight as ChevronIcon } from "lucide-react";
...
export function PhoneDeals() {
  ...
  const phones = products.filter((p) => p.subcategory === "Phones" || p.tags.some((t) => t.toLowerCase().includes("phone") || t.toLowerCase().includes("smartphone") || t.toLowerCase().includes("mobile"))).slice(0, 10);
    ...
    <section className="py-4 sm:py-6">
      <div className="mb-3 flex items-center justify-between sm:mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-foreground sm:text-lg">Phone Deals</h2>
        </div>
        <Link href="/category/electronics" className="text-xs font-semibold text-primary hover:text-primary/80 transition-colors flex items-center gap-0.5">
          View All
      ...
      <div className="relative group/carousel">
        ...
        <div ref={scrollRef} className="flex gap-3 overflow-x-auto cursor-grab active:cursor-grabbing pb-2 snap-x snap-mandatory scroll-smooth scrollbar-hide" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
              ...
              <div key={product.id} className="flex-shrink-0 w-[170px] sm:w-[200px] snap-start group select-none">
                <div className="relative rounded-2xl border border-border bg-card overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:border-primary/30">
                  <Link href={`/product/${product.id}`} className="block relative aspect-square bg-muted overflow-hidden">
                    ...
                    {discount > 0 && <span className="absolute top-2 left-2 rounded-full bg-accent px-2 py-0.5 text-[10px] font-bold text-white shadow-md">-{discount}%</span>}
          ...
          <Link href="/category/electronics" className="flex-shrink-0 w-[140px] sm:w-[160px] snap-start flex items-center justify-center rounded-2xl border border-dashed border-primary/40 bg-primary/5 hover:bg-primary/10 transition-colors select-none">
            <div className="text-center p-4">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">Phones</p>
            </div>
          </Link>
        </div>
      </div>
