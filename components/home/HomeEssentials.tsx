...
import { ChevronLeft, ChevronRight, Home, ChevronRight as ChevronIcon } from "lucide-react";
...
export function HomeEssentials() {
  ...
  const essentials = products.filter((p) => p.category === "home-living" || p.category === "groceries").slice(0, 10);
    ...
    <section className="py-4 sm:py-6">
      <div className="mb-3 flex items-center justify-between sm:mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-foreground sm:text-lg">Home Essentials</h2>
        </div>
        <Link href="/category/home-living" className="text-xs font-semibold text-primary hover:text-primary/80 transition-colors flex items-center gap-0.5">
          View All
      ...
      <div className="relative group/carousel">
        ...
        <div ref={scrollRef} className="flex gap-3 overflow-x-auto cursor-grab active:cursor-grabbing pb-2 snap-x snap-mandatory scroll-smooth scrollbar-hide" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
          ...
          <Link href="/category/home-living" className="flex-shrink-0 w-[140px] sm:w-[160px] snap-start flex items-center justify-center rounded-2xl border border-dashed border-primary/40 bg-primary/5 hover:bg-primary/10 transition-colors select-none">
            <div className="text-center p-4">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">Home & Living</p>
            </div>
          </Link>
        </div>
      </div>
