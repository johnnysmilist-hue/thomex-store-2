...
import { ChevronLeft, ChevronRight, TrendingUp, ChevronRight as ChevronIcon } from "lucide-react";
    ...
    <section className="py-4 sm:py-6">
      <div className="mb-3 flex items-center justify-between sm:mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-foreground sm:text-lg">Trending Now</h2>
        </div>
        <Link
          href="/products"
        ...
        >
          {trending.map((product) => {
            ...
            return (
              <div
                key={product.id}
                className="flex-shrink-0 w-[260px] sm:w-[280px] snap-start group select-none"
              >
                <div className="relative rounded-2xl border border-border bg-card overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:border-primary/30">
                  <Link href={`/product/${product.id}`} className="block relative aspect-[4/3] bg-muted overflow-hidden">
                    ...
                    />
                    {discount > 0 && (
                      <span className="absolute top-2.5 left-2.5 rounded-full bg-accent px-2 py-0.5 text-[10px] font-bold text-white shadow-md">
                        -{discount}%
                      </span>
                    )}
                    {product.freeShipping && (
                      <span className="absolute top-2.5 right-2.5 rounded-full bg-primary/90 px-2 py-0.5 text-[10px] font-bold text-white shadow-md backdrop-blur-sm">
                        Free Ship
                      </span>
                    )}
                    <button
                      ...
                      className={cn(
                        "absolute bottom-2.5 right-2.5 rounded-full p-2 shadow-md transition-all backdrop-blur-sm",
                        isWishlisted
                  ...
                  <div className="p-3">
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-1.5 rounded-xl bg-primary/10 py-2 text-xs font-semibold text-primary transition-all hover:bg-primary hover:text-primary-foreground active:scale-[0.98]"
                    >
          ...
          <Link
            href="/products"
            className="flex-shrink-0 w-[180px] sm:w-[200px] snap-start flex items-center justify-center rounded-2xl border border-dashed border-primary/40 bg-primary/5 hover:bg-primary/10 transition-colors select-none"
          >
            <div className="text-center p-4">
              <div className="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-primary/15">
                <ChevronIcon className="h-6 w-6 text-primary" />
              </div>
